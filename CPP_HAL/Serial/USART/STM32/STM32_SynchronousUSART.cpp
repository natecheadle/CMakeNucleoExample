#include "STM32_SynchronousUSART.h"

namespace CPP_HAL {
    STM32_SynchronousUSART::STM32_SynchronousUSART(
            ...
            SynchronousUSART(baudRate, wordLength, stopBits, parity, mode, flowControlMode, overSampling),
            m_RxPin(rxPin),
            m_TxPin(txPin) {
    ...
    bool STM32_SynchronousUSART::do_sendBytes(const uint8_t *pData, size_t size, std::chrono::milliseconds timeout) {
    ...
    bool STM32_SynchronousUSART::do_receiveBytes(unsigned char *pData, size_t size, std::chrono::milliseconds timeout) {
        return HAL_UART_Receive(&m_HUART, pData, static_cast<uint16_t>(size),
                                static_cast<uint32_t>(timeout.count())) == HAL_OK;
    }

    GPIO_InitTypeDef STM32_SynchronousUSART::populateGPIOStruct(const DIO_Pin<STM32_Pin> &pin) const {
    ...
    UART_HandleTypeDef STM32_SynchronousUSART::populateUARTStruct() const {
    ...
    USART_TypeDef *STM32_SynchronousUSART::GetUSARTInstance(Pin_ID rxPin, Pin_ID txPin) {
        switch (rxPin) {
            case Pin_ID::PA_3:
            case Pin_ID::PA_15:
            case Pin_ID::PB_4:
